---
title: @nfq/object-property-newline
rule_type: layout
related_rules:
- brace-style
- comma-dangle
...
An array of property names that are allowed to stay on a single line when the object contains only those keys. This is useful for screen-size maps.

If not provided, the rule reads `settings['@nfq'].objectPropertyNewline.ignoredProperties`.

Examples of **correct** code for the `{ "ignoredProperties": ["xs", "sm", "md"] }` option:

::: correct

```js
/*eslint @nfq/object-property-newline: ["error", { "ignoredProperties": ["xs", "sm", "md"] }]*/
...
```js
/*eslint @nfq/object-property-newline: "error"*/
...
```js
/*eslint @nfq/object-property-newline: ["error", { "allowAllPropertiesOnSameLine": true }]*/
